...
- (void)viewDidLoad
{
	...
	sortedModelArray = [[NSMutableArray alloc] init];
	
	for ( int i = 0; i < [modelArray count]; i++ ) {
		if ( [sortedModelArray count] == 0 ) {
			[sortedModelArray addObject:[modelArray objectAtIndex:0]];
		} else {
			Gyrus2Model *newModel = (Gyrus2Model *)[modelArray objectAtIndex:i];
			
			int j = 0;
			BOOL inserted = NO;
			
			for ( j = 0; j < [sortedModelArray count] && inserted == NO; j++ ) {
				Gyrus2Model *oldModel = (Gyrus2Model *)[sortedModelArray objectAtIndex:j];
				
				NSComparisonResult result = [oldModel.outcome compare:newModel.outcome];
				
				if ( result == NSOrderedDescending) {
					[sortedModelArray insertObject:newModel atIndex:j];
					inserted = YES;
				} else if ( result == NSOrderedSame) {
					NSComparisonResult result2 = [oldModel.author compare:newModel.author];
			...
			if ( inserted == NO ) {
				[sortedModelArray addObject:newModel];
			}
		}
	}
}
...
{
	Gyrus2Model *g2m = [sortedModelArray objectAtIndex:0];
...
{
	Gyrus2Model *g2m = (Gyrus2Model *)[sortedModelArray objectAtIndex:0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	Gyrus2Model *gyrus2model = [sortedModelArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	AppDelegate *delegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
	delegate.gyrus2model = (Gyrus2Model *)[sortedModelArray objectAtIndex:indexPath.row];
