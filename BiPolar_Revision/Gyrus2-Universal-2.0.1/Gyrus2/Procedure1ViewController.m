...
@implementation Procedure1ViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ( iPadDevice )
	{
		if ( [gyrus2model.procedure hasPrefix:@"Urethral Strictures"] )
			label.font = [UIFont fontWithName:@"Helvetica-Bold" size:19.0];
	}
	else
	{
		if ( [gyrus2model.procedure hasPrefix:@"Urethral Strictures"] )
	...
	label.text = [self stringForTVC:gyrus2model.procedure];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	Procedure2ViewController *procedure2VC = [[Procedure2ViewController alloc] initWithNibName:@"Procedure2ViewController" bundle:nil];
	...
	switch ( [segmentedControl selectedSegmentIndex] )
	{
		case 1:	// PK
			arrayStarts = sectionStartsPK;
			arrayModel = modelArrayPK;
			procedure2VC.navigationItem.prompt = [NSString stringWithFormat:@"%@ [%@]", iPadDevice ? @"PK® Technology and Olympus Resection in Saline: Bipolar Clinical Studies" : @"Olympus Clinical Studies", @"PK"];
			break;
			
		case 2:	// Olympus
			arrayStarts = sectionStartsOlympus;
			arrayModel = modelArrayOlympus;
			procedure2VC.navigationItem.prompt = [NSString stringWithFormat:@"%@ [%@]", iPadDevice ? @"PK® Technology and Olympus Resection in Saline: Bipolar Clinical Studies" : @"Olympus Clinical Studies", @"Olympus"];
			break;
			
		default: // All
			arrayStarts = sectionStarts;
			arrayModel = modelArray;
			procedure2VC.navigationItem.prompt = [NSString stringWithFormat:@"%@ [%@]", iPadDevice ? @"PK® Technology and Olympus Resection in Saline: Bipolar Clinical Studies" : @"Olympus Clinical Studies", @"PK & Olympus"];
			break;
	}
	
	procedure2VC.title = @"Procedure";
	...
	NSLog(@"start = %d, stop = %d, total = %d", start, stop, stop - start);	// jbh2
	
	for ( int i = start; i < stop; i++ )
		[passedArray addObject:[arrayModel objectAtIndex:i]];
	
	NSLog(@"[passedArray count] = %d", [passedArray count]);	// jbh2
	
	procedure2VC.modelArray = self.passedArray;
	
	[self.navigationController pushViewController:procedure2VC animated:YES];
}
...
- (void) readProcedureFile 
{
	...
	if ( ! (myfile = fopen([[[NSBundle mainBundle] pathForResource:@"procedure" ofType:@"txt"] UTF8String], "r")) )
	{
		fprintf(stderr, "Could not open \"procedure.txt\" for for reading\n");
		exit(EXIT_FAILURE);
	}
	...
	while (fgets(line1, sizeof line1, myfile) != NULL)
	{
		...
		gyrus2model.listType = PROCEDURE;
			...
			{ 
				...
				{ 
					...
					{ 
						...
						[self addProcedureField:fcount withString:[NSString stringWithFormat:@"%s", line3] toModel:gyrus2model];
...
{
//	NSLog(@"addProcedureField [%d] [%@]", fcount, line3);
	switch ( fcount )
	{
		case 0:
...
- (void) createSectionStarts:(NSMutableArray *)mySectionStarts fromModelArray:(NSMutableArray *)myModelArray
{
	...
	for ( int i = 1; i < [myModelArray count]; i++ )
	{
		...
		if ( ! [[g2mB.procedure capitalizedString] isEqualToString:[g2mA.procedure capitalizedString]] )
